...
export default function StreamingMessage({ content, isStreaming = false, onComplete }: StreamingMessageProps) {
  ...
  useEffect(() => {
    console.log('StreamingMessage effect:', { content: JSON.stringify(content), isStreaming, displayedLength: displayedContent.length, contentLength: content.length });
    ...
    if (content.length > displayedContent.length) {
      ...
      console.log('New content to animate:', JSON.stringify(newContent));
      const typingSpeed = 20; // milliseconds per character

      if (newContent.length > 0) {
        let charIndex = 0;
        const typeNextChar = () => {
          if (charIndex < newContent.length && isStreaming) {
            const char = newContent[charIndex];
            console.log('Adding character:', JSON.stringify(char), 'at index:', charIndex);
            setDisplayedContent(prev => {
              const newDisplayed = prev + char;
              console.log('New displayed content:', JSON.stringify(newDisplayed));
              return newDisplayed;
            });
            charIndex++;
  ...
  }, [content, isStreaming, onComplete]); // Removed displayedContent from dependencies
  ...
  return (
    <div className="prose prose-sm max-w-none dark:prose-invert prose-headings:text-gray-900 dark:prose-headings:text-gray-100 prose-p:text-gray-700 dark:prose-p:text-gray-300 prose-code:bg-gray-100 dark:prose-code:bg-gray-800 prose-pre:bg-gray-50 dark:prose-pre:bg-gray-900">
      {isStreaming ? (
        // During streaming, show plain text with basic formatting
        <div style={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>
          {displayedContent}
          <span className="inline-block w-2 h-4 bg-blue-500 ml-1 animate-pulse"></span>
        </div>
      ) : (
        // When streaming is complete, show full ReactMarkdown
        <ReactMarkdown
